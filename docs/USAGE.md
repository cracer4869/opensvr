...
3. 页面上：
   - 看到本机所有网卡 IP —— **服务在所有网卡上同时监听，一视同仁**，设备用任意与其管理口**同网段**的 IP 都能连（**点 IP 一键复制**）。临时插入网卡后列表约 3 秒自动刷新，也可点"⟳ 刷新网卡"立即扫描。
   - 点开 **FTP / SFTP / TFTP** 任一开关启动对应服务；端口可就地修改；顶部有**全部启动/全部停止**。
   - 账号密码明文显示（默认 `admin` / `admin`），可现场改。
   - **当前连接**面板：实时看哪些设备连着、传什么文件、已传字节。
   - **日志**面板：三协议的登录、上传/下载（含文件名）、删除/重命名/建目录都有记录，失败会标红并附原因（TFTP 协议本身无删除/重命名，故只有读写记录）。
...
- 可通过 `config.yaml` 的 `root_dir` 预设固定目录，例如
  `D:\Opening_file\当前开局`。
...
- **TFTP 提速**：下载（设备拉固件）默认提前连发 8 块、丢包 1 秒快速重传（TFTP 标准是逐块等确认、丢包停 5 秒，现网速度常被拖到 KB/s 级）。支持 blksize 协商（RFC 2348），设备侧可协商大块进一步提速（如思科 `ip tftp blocksize 1468`）；Windows 自带 tftp.exe 不支持协商，速度受限属正常。
...
6. **TFTP 偏慢**：TFTP 协议逐块确认，受往返时延/丢包影响远大于 FTP/SFTP。工具已默认连发+快速重传；大文件优先走 FTP/SFTP，或在设备侧调大 TFTP 块大小（如思科 `ip tftp blocksize 1468`）。
